...
*********************************************************************************************************
*/
#define THREADX_ENABLE              // 使用THREADX 线程

#if defined(THREADX_ENABLE)
#define  CPU_CFG_CRITICAL_METHOD    CPU_CRITICAL_METHOD_STATUS_LOCAL      // 防止报警告

#include "tx_api.h"
// TX_INTERRUPT_SAVE_AREA
//typedef  CPU_INT32U                     CPU_SR;                     /* Defines   CPU status register size (see Note #3b).   */

#define  CPU_SR_ALLOC()                 TX_INTERRUPT_SAVE_AREA 
#define  CPU_CRITICAL_ENTER()           TX_DISABLE
#define  CPU_CRITICAL_EXIT()            TX_RESTORE
#else
